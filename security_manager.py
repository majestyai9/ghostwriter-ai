"""
Security Manager for GhostWriter AI
...
class SecureKeyStorage:
    """
    Secure storage for API keys using Fernet symmetric encryption
    ...
    def _initialize_cipher(self, master_key: Optional[str] = None) -> Fernet:
        """
        Initialize the Fernet cipher with master key
        
        Args:
            master_key: Optional master key, otherwise loads/generates one
    ...
    def _derive_key_from_password(self, password: str) -> bytes:
        ...
        """
        salt = b'ghostwriter_salt_v1'  # In production, use random salt stored separately
        kdf = PBKDF2(
    ...
    def store_api_key(self, provider: str, api_key: str) -> None:
        ...
        logger.info(f"Securely stored API key for provider: {provider}")
...
class PathValidator:
    ...
    DANGEROUS_PATTERNS = [
        ...
        r'[\\/]',  # Directory separators in project IDs
        r'\x00',   # Null bytes
        ...
        for pattern in cls.DANGEROUS_PATTERNS:
            if re.search(pattern, project_id):
                raise PathTraversalError(
                    f"Invalid project ID: contains dangerous pattern '{pattern}'"
                )
    ...
    def validate_file_path(cls, file_path: str, base_dir: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if base_dir:
                ...
                if not str(absolute_path).startswith(str(base)):
                    raise PathTraversalError(
